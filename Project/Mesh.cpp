//Kyle Zielinski
//2/04/2021
//Mesh.cpp file that sets up how mesh objects will be used/handled.
#include "Mesh.h"
...
void Mesh::InitializeBuffers(Vertex* vertices, int numVertices, unsigned int* indices, int numIndices, Microsoft::WRL::ComPtr<ID3D11Device> device, bool calcTangents)
{
	...
	device->CreateBuffer(&vbd, &initialVertexData, vBuff.GetAddressOf());
	...
	device->CreateBuffer(&ibd, &initialIndexData, iBuff.GetAddressOf());

	this->indexCount = numIndices;
}

void Mesh::DrawUsingBuffs(Microsoft::WRL::ComPtr<ID3D11DeviceContext> context)
{
	//Make sure to set buffers in the input assembler, need this when drawing entities
	UINT stride = sizeof(Vertex);
	UINT offset = 0;
	context->IASetVertexBuffers(0, 1, vBuff.GetAddressOf(), &stride, &offset);
	context->IASetIndexBuffer(iBuff.Get(), DXGI_FORMAT_R32_UINT, 0);

	//Now we can finally draw the mesh *sigh* <---only saying this because I had to rework everything into Draw() in entity 
